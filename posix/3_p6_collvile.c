...
// Generacion paralela de offspring nuevo
iterator = 0;
for (int l = 0; l<NUM_THREADS; l++)
{
double *p;
pthread_create(&threads[l], NULL, cruce, (void*)(p));
}
...
// crea nueva poblacion
	for (int j = core_thread*POP_SIZE/NUM_THREADS; j<(core_thread+1)*POP_SIZE/NUM_THREADS; j+=2)
	{
		for (int i = 0; i < IND_SIZE; i++)
		{
...
// actualiza la info de los hijos
	for (int j = core_thread*POP_SIZE/NUM_THREADS; j<(core_thread+1)*POP_SIZE/NUM_THREADS; j+=2)
	{
		if(j<=2) break; //mantiente padres de generacion anterior
		s = rand()%IND_SIZE;
