// enfoque 1 correr 1 poblacion en cada kernel
//enfoque 2 evaluar cada individuo en 1 kernel
...
__global__ void evaluateKernel(double *individuo, const int chrom_size)
{
...
void evaluate()
{
	...
	{	double fit_val = 0.0; 		
		...
		{
			aux_Y += pop[j].genes[i]*pow(2 , (IND_SIZE-16-i)-1); 

		}
		aux_Y = (aux_Y/STEP);
		for (int i = IND_SIZE/2; i < IND_SIZE-8; i++)
		{
			aux_W += pop[j].genes[i]*pow(2 , (IND_SIZE-8-i)-1); 

		}
		aux_W = (aux_W/STEP);
		for (int i = IND_SIZE-8 ; i < IND_SIZE; i++)
		{
		...
		fit_val = pow(aux_X, 0.6) + pow(aux_Y, 0.6) - (6*aux_X) - (4*aux_W) + (3*aux_Z);
...
//revisa que el secondFittest no sea igual al 0 si este ya es el fittest
	for (int j = aux_fit_2+1; j < POP_SIZE; j++)
	{
//comprueba que el secondFittest no sea igual al ya obtenido fittest
		if (j != aux_fit_1 && aux_fittest[j] < aux_fittest[aux_fit_2]){
			aux_fit_2 = j;		
		}
	}	
	secondFittest = aux_fit_2;
//mutacion para diversidad
	if(abs(aux_fittest[aux_fit_1]-aux_fittest[aux_fit_2]) < 0.000001) mutate_var = 1;
	printf("El individuo minimo es %d y el segundo %d\n", fittest+1, secondFittest+1);
	imprimir_individuo(fittest);
	imprimir_individuo(secondFittest);
